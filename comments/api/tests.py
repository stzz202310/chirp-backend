...
class CommentAPITest(TestCase):
    ...
    def test_create(self):
        # 1. 匿名用户不可以创建 tweet
        response = self.anonymous_client.post(path=COMMENT_LIST_API)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

        # 2. 啥参数都没带不行
        response = self.taotao_client.post(path=COMMENT_LIST_API)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        # 3. 只带 tweet_id 不行
        response = self.taotao_client.post(path=COMMENT_LIST_API, data={'tweet_id': self.tweet_taotao.id})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        # 4. 只带 content 不行
        response = self.taotao_client.post(path=COMMENT_LIST_API, data={'content': '1'})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        # 5. content 太长不行
        response = self.taotao_client.post(path=COMMENT_LIST_API, data={
    ...
    def test_destroy(self):
        ...
        # 1. 匿名不可以删除
        response = self.anonymous_client.delete(path=url_comment_taotao)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

        # 2. 非{评论作者 OR 推特作者}不能删除
        response = self.zhuzhu_client.delete(path=url_comment_taotao)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.data['detail'], 'You do not have permission to delete this object.')

        # 3. 评论作者 可以删除
        response = self.taotao_client.delete(path=url_comment_taotao)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Comment.objects.count(), count - 1)

        # 4. 推特作者 可以删除
        response = self.taotao_client.delete(path=url_comment_zhuzhu)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update(self):
        ...
        # 1. 匿名不可以更新
        response = self.anonymous_client.put(path=url, data={'content': 'new'})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

        # 2. 非本人不能更新 [推特作者不能更新]
        response = self.taotao_client.put(path=url, data={'content': 'new'})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_list(self):
        # 1. 必须带 tweet_id
        response = self.anonymous_client.get(path=COMMENT_LIST_API)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_comments_count(self):
        ...
        self.create_comment(user=self.taotao, tweet=self.tweet_taotao)
        ...
        self.create_comment(user=self.zhuzhu, tweet=self.tweet_taotao)
        self.create_newsfeed(user=self.zhuzhu, tweet=self.tweet_taotao)
    ...
    def test_comments_count_with_cache(self):
        ...
        # 3. 增加 第3条评论
        response = self.zhuzhu_client.post(path=COMMENT_LIST_API, data=data)
        ...
        #    comments.api.listeners: if not created: return
