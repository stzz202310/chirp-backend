...
GET /api/users/{user_id}/       ReadOnlyModelViewSet
...
PUT /api/profiles/{profile_id}/ update  data={'nickname':, 'avatar':}   viewsets.mixins.UpdateModelMixin 

POST    /api/comments/                  create  data = {'tweet_id':, 'content':,}, 'user_id': request.user.id
PUT     /api/comments/{comment_id}/     update  data = {'content':,}
DELETE  /api/comments/{comment_id}/     destroy
GET /api/comments/?tweet_id=1           list ✅ params=['tweet_id',]    {tweet_id=1}tweet 的所有 comments
GET /api/tweets/{tweet_id}/comments/    list ❌ 
1. /api/comments/?tweet_id=1 更restful, 想要获得comments 所以应该是 /api/comments/, tweed_id只是筛选项
2. /api/tweets/1/最好是动词/

============================================================================================
POST /api/friendships/{user_id=1}/follow/     当前用户request.user 关注 follow   user_id=1 的用户
...
PUT  /api/notifications/{notification_id}/  update params=['unread',], 标记一个 notification 为已读或者未读
...
1. URL 查询参数本质是 key=value [❌不支持 ?id<100, 因为 URL 参数无法表达运算符] 
2. Django ORM 支持双下划线查找 [field__lookup = value]
    Tweet.objects.filter(id__lt=10)                 WHERE id < 10
    Tweet.objects.filter(content__icontains="abc")  WHERE name ILIKE '%abc%'
